...
2. **You can spawn sub-agents in parallel** (e.g. a Task tool with an `sdpm-composer`
   ...
   Each query MUST include: deck_id (absolute path), assigned slide slugs, and a pointer
   to specs/. ASCII-only queries. If the composer agent is not registered, STOP — tell
   the user to install it (e.g. `make install-kiro` for Kiro CLI,
   `/plugin install sdpm@sdpm` for Claude Code) and restart the session.
...
2. **Verification**: view the post-review renders. If you composed sequentially
   ...
   calls; if you dispatched composers (their tool results are not visible to
   you), call `get_preview(deck_id, slugs=[...all slugs...])` — verification is
   the one exception to "do not call preview tools directly". Look for
   individual-slide defects that remain: text overflow, element overlap,
   broken layout, alignment issues.
