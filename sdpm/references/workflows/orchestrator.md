...
| `specs/art-direction.html` + `deck.json` | Choose a template (`list_templates()`, `analyze_template(...)`) and a style (`list_styles()`), then `apply_style(deck_id, style, template)` — it writes both files and returns `missing` for deck.json fields it could not derive (e.g. `defaultTextColor` when the style has no `--color-text`); set those yourself. Frozen afterwards: parallel composers depend on them. |
