...
| `specs/outline.md` | Parsed by the web UI, so the format is fixed: `## Heading` for a chapter; `- [slug] title` for a slide, where the one-sentence claim becomes its headline (kebab-case slug → `slides/<slug>.json`); then indented `  - key: value` sub-items: `body` describes in natural-language prose what the slide says and how it is expressed (not verbatim slide text; the composer may refine wording and add supporting detail), `visual` names the visual form and elements to show rather than prescribing layout (for example, a 4-row before-vs-after table, three-step horizontal flow, bar chart of X by Y, or A→B→C architecture diagram), and `evidence` points into the brief's Sources (URL + section or attachment path + page). Sub-items are the normal form: write all three for every slide so a reviewer can foresee the slide and the composer has its build intent; in dialogue, the user reviews them slide by slide. `[TBD]` marks missing information. Slugs sharing a visual base share a prefix (`demo-1`, `demo-2`). |
