...
| `specs/art-direction.html` + `deck.json` | Written **before the outline**. Choose a template and a style from the lists `start_presentation()` returned (`analyze_template(...)` for a template's layouts; `list_styles(include_all=True)` for styles hidden by pins), then `apply_style(deck_id, style, template)` — it writes both files and returns the resulting `deck.json` with where each value came from; edit it with `run_python` if a derived value is not what the deck needs. Frozen afterwards: parallel composers depend on them. If the user's message carries `@template:<name>` or `@style:<name>` (quoted when the name has spaces), those are explicit choices: use them as given, in `fast` mode too, instead of picking yourself. If a named template or style does not exist, do not substitute — say so and ask. Then read the style's Part 2 (Rules) and Part 3 (Message & Outline) before writing the outline: they fix the deck's length range, per-slide density, title grammar, chapter shape and preferred visual forms, and an outline written without them has to be rewritten once the style is applied. |
