...
logger = logging.getLogger("png-worker.autofit")
...
def extract_scaling(pptx_path: Path) -> dict[tuple[int, int], tuple[float, float]]:
    ...
    Uses (slide_index, shape_index) as key. Shape ordering is preserved
    by LibreOffice on re-save.
    ...
    for si, slide in enumerate(prs.slides):
        for shi, shape in enumerate(slide.shapes):
            ...
            if norm is None:
                continue
            font_scale: float = int(norm.get("fontScale", "100000")) / 100000.0
            ln_spc_reduction: float = int(norm.get("lnSpcReduction", "0")) / 100000.0
...
def unlock_autofit(
    ...
    Args:
        ...
        scaling: Dict from extract_scaling() mapping (slide_index, shape_index)
                 to (fontScale, lnSpcReduction). Used only for logging.

    Returns:
        True if any shapes were modified, False otherwise.
    """
